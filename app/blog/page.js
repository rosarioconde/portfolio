...
export default async function Blog() {
 ...
 const products = await getGoogleSheetsData(range);

 return <div className="mx-auto max-w-2xl max-w-6xl py-16 p-4">
        <div className="flex flex-col justify-center items-center">
          <div className="m-8">
            {products.map((product)  => {
              const [id, category, title, description, image, timeRead, datePublication ] = product;
                return (
                <div key={id} className="m-4  flex flex-col md:flex-row md:space-x-5 space-y-3 md:space-y-0 rounded-xl shadow-lg p-3 max-w-xs md:max-w-3xl mx-auto border border-white bg-white">
                  <div className="w-full md:w-1/3 bg-white grid place-items-center">
                    ...
                    <div className="w-full md:w-2/3 bg-white flex flex-col space-y-2 p-3">
                      <div className="flex justify-between item-center">
                        <p className="text-gray-500 font-medium hidden md:block">{datePublication}</p>
